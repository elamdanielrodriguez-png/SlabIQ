...
You have ZOOM-IN crops of each of the 8 corner/edge zones labeled "ZOOM-IN FRONT: zone". For each corner zone, an additional TIP close-up (labeled "FRONT: corner-XX (TIP — extreme close-up of corner point)") is also provided — this is an extreme zoom on just the corner point itself, the exact region that determines PSA 9 vs PSA 10. The TIP image is your primary evidence for detecting whitening, tip rounding, soft points, or micro-fraying. If the TIP shows ANY issue, grade the corner as "differs" regardless of what the wide crop shows. For each zone, examine BOTH the front and back crops (and TIP if provided). The front face is primary — grade primarily from what you see on the front. Back damage counts but is weighted lower:
  - Back "obvious" damage → contributes as "visible" severity
  - Back "visible" damage → contributes as "microscopic" severity
  - Back "microscopic" damage → does not affect the zone severity
Take the worse of the front severity and the softened back severity as the zone's final reported severity. A card with a perfect front but a visibly chipped back corner gets "microscopic" on that corner — not clean, but not the full hit it would be if on the front.
